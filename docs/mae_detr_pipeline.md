...
│   └── neighborhood_encoder.py  # (Track B) GATv2 neighbourhood encoder — not used here
│
├── training/
...
│   ├── visualize_pretrain.py  # Render MAE reconstructions from a checkpoint
│   └── visualize_detections.py  # Draw predicted vs. ground-truth boxes
│
├── utils/
...
Open `notebooks/colab_train.ipynb` in Google Colab. The notebook:
...
This pipeline was developed in Prof. Ngan Huang's lab for cardiovascular tissue-on-a-chip research.
