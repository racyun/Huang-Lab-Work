...
def main() -> None:
    ...
    for name, clf in [
        ...
        results[name] = {"accuracy": acc, "std": float(scores.std())}
    ...
    if ratio >= 2.0 and best_acc >= 0.5:
        verdict = "LEAK — condition is strongly recoverable; ADD the adversarial head"
    elif ratio >= 1.5:
        verdict = "PARTIAL — some condition signal present; consider the adversarial head"
    else:
        verdict = "CLEAN — condition not recoverable; adversary NOT needed"
    print(f"\nVerdict: {verdict}")
